...
SUBTYPE POSITION IS INTEGER RANGE 0 .. 150;
	...
	--t2 ? t4
	--
	PROCEDURE DFS (TOWER_FIRST_NAME : STRING; TOWER_SECOND_NAME : STRING) IS
	...
	BEGIN
		...
		IF CURRENT.SOURCE = TO_UNBOUNDED_STRING(TOWER_SECOND_NAME) THEN -- IF YOU ARE SEARCHING THE TOWER YOU WANT
			ADA.TEXT_IO.PUT_LINE("PASS BECAUSE " & TOWER_SECOND_NAME & " IS EQUAL TO " & TO_STRING(CURRENT.SOURCE)); -- YOU WIN LETS GOOOOO
		
		-- RECURSE
		ELSE
			ADA.TEXT_IO.PUT("ON:" & TO_STRING(CURRENT.SOURCE) & " ");
			FOR TOWER OF CURRENT.LINK LOOP -- FOR ALL LINKS IN SOURCE
				IF NOT TOWER.CHECKED THEN -- IF THE LINK HASN'T BEEN CHECKED
					DFS(TO_STRING(TOWER.SOURCE), TOWER_SECOND_NAME); -- SEARCH THE LINK
				END IF;
			END LOOP;
		END IF;
		
		ADA.TEXT_IO.PUT_LINE("GOT TO END OF DFS");
	END DFS;
	...
	BEGIN
		...
		ELSIF COMMAND = '?' THEN
			FIRST_TOWER_EXISTS := EXISTS_IN_SOURCE(TOWER_FIRST_NAME);
			SECOND_TOWER_EXISTS := EXISTS_IN_SOURCE(TOWER_SECOND_NAME);
			...
			ELSIF TOWER_FIRST_NAME = TOWER_SECOND_NAME THEN
				ADA.TEXT_IO.PUT_LINE("+ " & TOWER_FIRST_NAME & " => " & TOWER_SECOND_NAME);
			ELSE
				DFS(TOWER_FIRST_NAME, TOWER_SECOND_NAME);
